  ...
  if (account) {
    return (
      <Web3StatusConnected id="web3-status-connected" onClick={toggleWalletModal} pending={hasPendingTransactions}>
        {hasPendingTransactions ? (
          <RowBetween>
            <TextAddress>{pending?.length} Pending</TextAddress> <Loader stroke="white" />
          </RowBetween>
        ) : (
          <>
            {hasSocks ? SOCK : null}
            <TextAddress>{ENSName || shortenAddress(account)}</TextAddress>
          </>
        )}
    ...
    return (
      <Web3StatusError onClick={toggleWalletModal}>
        <NetworkIcon />
        <TextAddress>{error instanceof UnsupportedChainIdError ? t('wrong_network') : t('error')}</TextAddress>
      </Web3StatusError>
    )
  } else {
    return (
