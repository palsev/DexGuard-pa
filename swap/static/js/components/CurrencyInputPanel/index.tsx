...
const InputPanel = styled.div<{ hideInput?: boolean }>`
  ...
  background-color: ${({ theme, hideInput }) => (hideInput ? 'transparent' : theme.bg0)};
  z-index: 1;
...
const CurrencySelect = styled(ButtonGray)<{ selected: boolean; hideInput?: boolean }>`
  ...
  color: #fff;
  border-radius: 16px;
  box-shadow: ${({ selected }) => (selected ? 'none' : '0px 0px 5px 3px rgba(243, 30, 90, 0.5);')};
  ...
  border: ${({ selected }) => (!selected && '1px solid #f31e5a')};
  :focus,
  :hover {
    // background-color: ${({ selected, theme }) => (selected ? darken(0.05, theme.bg0) : darken(0.05, theme.primary1))};
    background: #202230;
  }
`
